...
export const CategoryDetail:FC<Props> = ({venue}) => {
    console.log(venue);
      ...
      <Grid item xs={12} sm={12} sx={{ border:0, width:'100%' }} >
        <Box display='flex' flexDirection='row'  >
          ...
          <Box display='flex' flexDirection='column' sx={{  width: '40%', p:3}} >
            ...
            > { venue.venuename}</Typography>
            <Box display='flex' flexDirection='row' >
              <Typography sx={{width: '40%'}}  variant='subtitle1' > ISBN </Typography>
              <Typography sx={{width: '60%'}}> {venue.venueseats} </Typography>
            </Box>
            <Box display='flex' flexDirection='row'>
            ...
            <Box display='flex' flexDirection='row'>
              ...
              <Typography sx={{width: '60%'}}> {venue.venuecity} </Typography>
            </Box>
          </Box>
          <Box  sx={{ display:'flex', flexDirection:'column', justifyContent:'space-around', alignItems:'center'}}>
            <Typography variant='h4' component='h4'> { venue.venuename } </Typography>
        ...
        <Box sx={{ width: '100%', typography: 'body1' }}>
          <TabContext value={value}>
            ...
            <TabPanel value="1">{venue.venuename}</TabPanel>
            <TabPanel value="2">{venue.venuestate}</TabPanel>
            <TabPanel value="3">{venue.venueseats}</TabPanel>
          </TabContext>
        </Box>
        {/* <Box sx={{ width: '100%' }}>
